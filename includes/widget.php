...
{
    register_widget('guilro_petitions_petition_widget');
}
...
class guilro_petitions_petition_widget extends WP_Widget
{
    public function __construct()
    {
        ...
        if (!is_admin() && is_active_widget(false, false, $this->id_base, true)) {
            ...
            // attempt to load cusom theme (petition-widget.css)
                ...
                if (is_child_theme()) {
                    ...
                    if (file_exists($child_theme_path)) {
                        wp_enqueue_style('guilro_petitions_widget_css', $child_theme_url);
                    }
                    // else try to load style from parent theme folder
                    else {
                        wp_enqueue_style('guilro_petitions_widget_css', $parent_theme_url);
                    }
                }
            ...
            wp_localize_script('guilro_petitions_widget_js', 'guilro_petitions_widget_js', $params);
        }
    }
    ...
    public function form($instance)
    {
        ...
        echo '<p><label>'.__('Title', 'guilro_petitions').':</label><br /><input class="widefat" type="text" name="'.$this->get_field_name('title').'" value="'.stripslashes($instance['title']).'"></p>';
        echo '<p><label>'.__('Sharing URL', 'guilro_petitions').':</label><br /><input class="widefat" type="text" name="'.$this->get_field_name('sharing_url').'" value="'.stripslashes($instance['sharing_url']).'"></p>';
        echo '<p><label>'.__('Call to Action', 'guilro_petitions').':</label><br /><textarea maxlength="140" class="widefat" name="'.$this->get_field_name('call_to_action').'">'.$instance['call_to_action'].'</textarea></p>';
        echo '<p><label>'.__('Petition', 'guilro_petitions').':</label><br /><select class="widefat" name="'.$this->get_field_name('petition_id').'">';
    ...
    public function widget($args, $instance)
    {
        ...
        $greeting = ($petition->greeting != '' && $petition->sends_email == 1) ? '<p><span class="guilro-petitions-widget-greeting">'.$petition->greeting.'</span></p>' : '';
        ...
        if ($get_petition) {
            ...
            $petition_widget = '
				...
				<div class="guilro-petitions-widget-wrap">
					<h3>'.stripslashes(esc_html($title)).'</h3>
					<p>'.stripslashes(esc_html($call_to_action)).'</p>
					<div class="guilro-petitions-widget-button-wrap">
						<a rel="guilro-petitions-widget-popup-wrap-'.$petition->id.'" class="guilro-petitions-widget-button"><span>'.$options['button_text'].'</span></a>
					</div>';
            if ($options['display_count'] == 1) {
                $petition_widget .= '
					<div class="guilro-petitions-widget-progress-wrap">
						<div class="guilro-petitions-widget-signature-count">
							<span>'.number_format($petition->signatures).'</span> '._n('signature', 'signatures', $petition->signatures, 'guilro_petitions').'
						</div>
						'.guilro_petitions_SpeakOut::progress_bar($petition->goal, $petition->signatures, 150).'
					</div>';
            }
            $petition_widget .= '
				</div>

				<div id="guilro-petitions-widget-windowshade"></div>
				<div id="guilro-petitions-widget-popup-wrap-'.$petition->id.'" class="guilro-petitions-widget-popup-wrap">
					<h3>'.stripslashes(esc_html($petition->title)).'</h3>
					<div class="guilro-petitions-widget-close"></div>';
            if ($petition->is_editable == 1) {
                $petition_widget .= '
					<div class="guilro-petitions-widget-message-wrap">
						<p class="guilro-petitions-greeting">'.$petition->greeting.'</p>
						<textarea name="guilro-petitions-widget-message" id="guilro-petitions-widget-message-'.$petition->id.'" class="guilro-petitions-widget-message">'.stripslashes(esc_textarea($petition->petition_message)).'</textarea>
						<p class="guilro-petitions-caps">['.__('signature', 'guilro-petitions').']</p>
					</div>';
            } else {
                $petition_widget .= '
					<div class="guilro-petitions-widget-message-wrap">
						<div class="guilro-petitions-widget-message">
							<p class="guilro-petitions-greeting">'.$petition->greeting.'</p>
            ...
            $petition_widget .= '
					<div class="guilro-petitions-widget-form-wrap">
						<div class="guilro-petitions-widget-response"></div>
						<form class="guilro-petitions-widget-form">
							<input type="hidden" id="guilro-petitions-widget-posttitle-'.$petition->id.'" value="'.esc_attr(urlencode(stripslashes($petition->title))).'" />
							<input type="hidden" id="guilro-petitions-widget-shareurl-'.$petition->id.'" value="'.esc_attr(urlencode(stripslashes($instance['sharing_url']))).'" />
							<input type="hidden" id="guilro-petitions-widget-tweet-'.$petition->id.'" value="'.guilro_petitions_SpeakOut::twitter_encode($petition->twitter_message).'" />
							<input type="hidden" id="guilro-petitions-widget-lang-'.$petition->id.'" value="'.$wpml_lang.'" />';

            if ($expired) {
                $petition_widget .= '
							<p><strong>'.$options['expiration_message'].'</strong></p>
							<p>'.__('End date', 'guilro_petitions').': '.date('M d, Y', strtotime($petition->expiration_date)).'</p>
							<p>'.__('Signatures collected', 'guilro_petitions').': '.$petition->signatures.'</p>';
                if ($petition->goal != 0) {
                    $petition_widget .= '
							<p><div class="guilro-petitions-expired-goal"><span>'.__('Signature goal', 'guilro_petitions').':</span> '.$petition->goal.'</div></p>';
                }
            } else {
                $petition_widget .= '
							<div class="guilro-petitions-widget-full">
								<label for="guilro-petitions-widget-first-name-'.$petition->id.'" class="required">'.__('First Name', 'guilro_petitions').'</label>
								<input name="guilro-petitions-widget-first-name" id="guilro-petitions-widget-first-name-'.$petition->id.'" value="'.$userdata['firstname'].'" type="text" />
							</div>
							<div class="guilro-petitions-widget-full">
								<label for="guilro-petitions-widget-last-name-'.$petition->id.'" class="required">'.__('Last Name', 'guilro_petitions').'</label>
								<input name="guilro-petitions-widget-last-name" id="guilro-petitions-widget-last-name-'.$petition->id.'" value="'.$userdata['lastname'].'" type="text" />
							</div>
							<div class="guilro-petitions-widget-full">
								<label for="guilro-petitions-widget-email-'.$petition->id.'" class="required">'.__('Email', 'guilro_petitions').'</label>
								<input name="guilro-petitions-widget-email" id="guilro-petitions-widget-email-'.$petition->id.'" value="'.$userdata['email'].'" type="text" />
							</div>';
                if ($petition->requires_confirmation) {
                    $petition_widget .= '
							<div class="guilro-petitions-widget-full">
								<label for="guilro-petitions-widget-email-confirm-'.$petition->id.'" class="required">'.__('Confirm Email', 'guilro_petitions').'</label>
								<input name="guilro-petitions-widget-email-confirm" id="guilro-petitions-widget-email-confirm-'.$petition->id.'" value="" type="text" />
							</div>';
                }
                if (in_array('street', $petition->address_fields)) {
                    $petition_widget .= '
							<div class="guilro-petitions-widget-full">
								<label for="guilro-petitions-widget-street-'.$petition->id.'">'.__('Street', 'guilro_petitions').'</label>
								<input name="guilro-petitions-widget-street" id="guilro-petitions-widget-street-'.$petition->id.'" maxlength="200" type="text" />
							</div>';
                }
                if (in_array('city', $petition->address_fields)) {
                    $petition_widget .= '
							<div class="guilro-petitions-widget-half">
								<label for="guilro-petitions-widget-city-'.$petition->id.'">'.__('City', 'guilro_petitions').'</label>
								<input name="guilro-petitions-widget-city" id="guilro-petitions-widget-city-'.$petition->id.'" maxlength="200" type="text">
							</div>';
                }
                if (in_array('state', $petition->address_fields)) {
                    $petition_widget .= '
							<div class="guilro-petitions-widget-half">
								<label for="guilro-petitions-widget-state-'.$petition->id.'">'.__('State / Province', 'guilro_petitions').'</label>
								<input name="guilro-petitions-widget-state" id="guilro-petitions-widget-state-'.$petition->id.'" maxlength="200" type="text" list="guilro-petitions-states" />
                ...
                if (in_array('postcode', $petition->address_fields)) {
                    $petition_widget .= '
							<div class="guilro-petitions-widget-half">
								<label for="guilro-petitions-widget-postcode-'.$petition->id.'">'.__('Post Code', 'guilro_petitions').'</label>
								<input name="guilro-petitions-widget-postcode" id="guilro-petitions-widget-postcode-'.$petition->id.'" maxlength="200" type="text">
							</div>';
                }
                if (in_array('country', $petition->address_fields)) {
                    $petition_widget .= '
							<div class="guilro-petitions-widget-half">
								<label for="guilro-petitions-widget-country-'.$petition->id.'">'.__('Country', 'guilro_petitions').'</label>
								<input name="guilro-petitions-widget-country" id="guilro-petitions-widget-country-'.$petition->id.'" maxlength="200" type="text" list="guilro-petitions-widget-countries" />
								<datalist id="guilro-petitions-widget-countries">
                ...
                if ($petition->displays_custom_field == 1) {
                    $petition_widget .= '
							<div class="guilro-petitions-widget-full">
								<label for="guilro-petitions-widget-custom-field-'.$petition->id.'">'.stripslashes(esc_html($petition->custom_field_label)).'</label>
								<input name="guilro-petitions-widget-custom-field" id="guilro-petitions-widget-custom-field-'.$petition->id.'" maxlength="400" type="text">
							</div>';
                }
                if ($petition->displays_optin == 1) {
                    $optin_default = ($options['optin_default'] == 'checked') ? ' checked="checked"' : '';
                    $petition_widget .= '
							<div class="guilro-petitions-widget-optin-wrap">
								<input type="checkbox" name="guilro-petitions-widget-optin" id="guilro-petitions-widget-optin-'.$petition->id.'"'.$optin_default.' />
								<label for="guilro-petitions-widget-optin-'.$petition->id.'">'.stripslashes(esc_html($petition->optin_label)).'</label>
							</div>';
                }
                $petition_widget .= '
							<div class="guilro-petitions-widget-submit-wrap">
								<div id="guilro-petitions-widget-ajaxloader-'.$petition->id.'" class="guilro-petitions-widget-ajaxloader" style="visibility: hidden;">&nbsp;</div>
								<a name="'.$petition->id.'" class="guilro-petitions-widget-submit"><span>'.stripslashes(esc_html($options['button_text'])).'</span></a>
							</div>
						</form>
						<div class="guilro-petitions-widget-share">
							<p><strong>'.stripslashes(esc_html($options['share_message'])).'</strong></p>
							<p>
							<a class="guilro-petitions-widget-facebook" href="#" title="Facebook"><span></span></a>
							<a class="guilro-petitions-widget-twitter" href="#" title="Twitter"><span></span></a>
							</p>
